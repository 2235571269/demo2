...
class Ucenter extends IController implements userAuthorization
{
    ...
    //     } else {
    //         $title='好友邀请';
    //         $url="http://localhost/iWebShop5.5/index.php?controller=simple&action=reg&id=".urlencode(base64_encode($user_id));
    //         $content="您的好友邀请您注册账号，请点击连接:".$url;
    //         if ($smtp->send($email,$title,$content)) {
    //             $result=array('isError'=>false,'message'=>'发送成功');
    //         }else{
    //             $result=array('isError'=>true,'message'=>'发送失败,请确认您填写的邮箱是否正确');
    //         }
    //         echo JSON::encode($result);
    //     }
    // }
    ...
    function inviteMail()
    {
        ...
        if ($error=$smtp->getError()) {
            ...
            $title='123';
            $url='123'.urlencode(base64_encode($user_id));
            $content='123'.$url;
            if ($smtp->send($email,$title,$content)) {
                $result=array('isError'=>false,'message'=>'发送成功');
            }else{
