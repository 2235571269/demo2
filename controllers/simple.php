...
class Simple extends IController
{
    ...
    function callback(){
        ...
        $data = $_POST;
        $appid = 101763064;
        $appkey = "4c24d630ca9314783d2e7f207de68c16";
        $data = file_get_contents("https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=$appid&client_secret=$appkey&code=$data[code]&redirect_uri=http://www.iwebshoping.com/index.php?r=site/callback");
        $data = "access_token=B68C47C254F3C34B792B4A41A73E9E13&expires_in=7776000&refresh_token=A5AF3A6374E95D3EF143CE1A4C80278B";
        $access_token = substr($data,strpos($data,'=')+1,32);
        $data = file_get_contents("https://graph.qq.com/oauth2.0/me?access_token=$access_token");
        $openid = substr($data,strpos($data, '"', 44 )+1,32);
        $data = file_get_contents("https://graph.qq.com/user/get_user_info?access_token=$access_token&oauth_consumer_key=$appid&openid=$openid");
	...
	function login()
	{
        if ($_COOKIE['errorLogin']=='0') {
            $this->setError('登录信息有误！');
            setcookie('errorLogin',null);
        }
		//如果已经登录，就跳到ucenter页面
		if($this->user)
		{
		...
		{
			$this->redirect('login',['error'=>$error]);
		}
	}
   ...
   //  }
    ...
    function regTelInfo()
    {
        ...
        if ($data['mobile_code'] == $code) {
            ...
            $sql="insert into iwebshop_user(username,password,tel) value('$tel','$password','$tel')";
           $res=$dbh->exec($sql);
           if ($res) {
            $msg = "恭喜您~注册成功";
               $this->redirect('/site/success?message='.urlencode($msg));
    ...
    function reg(){
        ...
        if ($id!='') {
            ...
            $userData=$user->getObj('id='.$user_id,'username');
    ...
    function cart3()
    {
		...
		//最终订单金额计算
        $orderData = $countSumObj->countOrderFee($goodsResult,isset($province) ? $province : "",isset($delivery_id) ? $delivery_id : "",$taxes);
    ...
    public function bind_exists_user()
    {
    	...
    	if($userRow = plugin::trigger("isValidUser",array($login_info,md5($password))))
    	{
    		$oauthUserObj = new IModel('oauth_user');
             ...
             //判断唯一性
        $dbh = new \PDO('mysql:host=127.0.0.1;dbname=iwebshop', 'root','root');
            $find="select * from iwebshop_oauth_user where user_id=".$userRow['id'];
            // echo $sql;
           $req=$dbh->query($find);
           $findInfo=$req->fetch(2);
           if ($findInfo) {
              $this->setError("该用户已被绑定");
            $_GET['bind_type'] = 'exists';
            $this->redirect('bind_user',false);
            Util::showMessage("该用户已被绑定");
           }
	...
	public function seller_reg()
	{
		...
		$ak='tsyGQXzhWIU4eYNkXyI47LjeK5Og5Vxf';
