...
$(function(){
	//虚拟商品更新
	orderFormInstance.goodsType("<?php echo $this->goodsType;?>");
...
<section class="cart_2">
	...
	<form action='<?php echo IUrl::creatUrl("/simple/cart3");?>' method='post' name='order_form' onsubmit='return orderFormInstance.isSubmit()'>
		...
		<section class="cart_item" id="addressBox">
				...
				<ul class="addr_list" id="addr_list">
					...
					<li id="addressItem<?php echo isset($item['id'])?$item['id']:"";?>">
						<label>
							...
							<?php echo isset($item['accept_name'])?$item['accept_name']:"";?> <?php echo isset($item['province_str'])?$item['province_str']:"";?> <?php echo isset($item['city_str'])?$item['city_str']:"";?> <?php echo isset($item['area_str'])?$item['area_str']:"";?> <?php echo isset($item['address'])?$item['address']:"";?> <?php echo isset($item['mobile'])?$item['mobile']:"";?>
						</label>
            ...
            <?php if($this->takeselfList){?>
			<div class="cart_item_addr" style="margin-top:10px">
			    <h4>门店自提点</h4>
				<ul class="addr_list" id="takeself_list">
					<?php foreach($items=$this->takeselfList as $key => $item){?>
					<input type='hidden' name='accept_name' value='<?php echo isset($item['accept_name'])?$item['accept_name']:"";?>' />
					<input type='hidden' name='accept_mobile' value='<?php echo isset($item['accept_mobile'])?$item['accept_mobile']:"";?>' />
					<li>
						<label><input class="radio" name="takeself" type="radio" value="<?php echo isset($item['id'])?$item['id']:"";?>" onclick="orderFormInstance.takeselfSelected(<?php echo isset($item['id'])?$item['id']:"";?>);" /><?php echo isset($item['name'])?$item['name']:"";?> <?php echo isset($item['province_str'])?$item['province_str']:"";?> <?php echo isset($item['city_str'])?$item['city_str']:"";?> <?php echo isset($item['area_str'])?$item['area_str']:"";?> <?php echo isset($item['address'])?$item['address']:"";?> <?php echo isset($item['accept_name'])?$item['accept_name']:"";?> <?php echo isset($item['accept_mobile'])?$item['accept_mobile']:"";?></label> [<a href="javascript:orderFormInstance.selectTakeself(<?php echo isset($item['id'])?$item['id']:"";?>);" style="color:#005ea7;">修改自提点</a>]
					</li>
					<?php }?>
				</ul>
			...
			<div class="cart_item_pay">
				<table width="100%" class="border_table">
					...
					<tr>
						<th>
							<label>
								<input class="radio" name="payment"  title="<?php echo isset($item['name'])?$item['name']:"";?>" type="radio" value="<?php echo isset($item['id'])?$item['id']:"";?>" />
								<img src='<?php echo IUrl::creatUrl("plugins".$item['logo']."");?>' width='160px' />
								<?php echo isset($item['name'])?$item['name']:"";?>
							</label>
						</th>
