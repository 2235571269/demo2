...
<?php $goodsObj = search_goods::find($defaultWhere,20);$resultData = $goodsObj->find();?>

<div class="bigweb">
<section class="breadcrumb">
	<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> » 搜索"<?php echo $this->word;?>"
</section>

<section class="web">
	<section class="prolist_main">
		<!-- 搜索结果所在栏目 -->
		<?php $catExt = Api::run('getGoodsCategoryExtend',array('#word#',$this->word))?>
		<?php if($catExt){?>
		<section class="search_class">
			<strong>搜索结果分类：</strong>
			<div class="search_class_content">
				<?php foreach($items=$catExt as $key => $item){?>
				<?php $tmpWord=urlencode($this->word);?>
				<a href="<?php echo IUrl::creatUrl("/site/search_list/word/".$tmpWord."/cat/".$item['id']."");?>"><?php echo isset($item['name'])?$item['name']:"";?>(<?php echo isset($item['num'])?$item['num']:"";?>)</a>
				<?php }?>
			</div>
		</section>
		<?php }?>
