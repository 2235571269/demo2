...
<section class="uc_table">
	<table>
		...
		<tbody>
		    <?php $queryPropList = Api::run('getPropList')?>
			<?php foreach($items=$queryPropList->find() as $key => $item){?>
			<tr>
				<td><?php echo isset($item['card_name'])?$item['card_name']:"";?></td>
				<td><?php echo isset($item['value'])?$item['value']:"";?>元</td>
				<td><?php echo substr($item['end_time'],0,16);?></td>
				<td class="blue"><?php echo ticket::statusText(ticket::status($item));?></td>
			</tr>
			<?php }?>
		</tbody>
	</table>
...
<section class="user_form">
	<?php foreach($items=Api::run('getTicketList') as $key => $item){?>
		...
		<div class="style">
			<div class="info-box">
				...
				<div class="coupon-money">
					<div class="lay of">￥<em><?php echo isset($item['value'])?$item['value']:"";?></em></div>
					<div class="lay">
						<p class="tit">需要<?php echo isset($item['point'])?$item['point']:"";?>积分</p>
						<p class="demand">有效日期：<?php echo isset($item['start_time'])?$item['start_time']:"";?>至<?php echo isset($item['end_time'])?$item['end_time']:"";?></p>
					</div>
				</div>
			</div>
			<a href="javascript:;" class="get-btn">
				<input type="hidden" name='ticket_id' value="<?php echo isset($item['id'])?$item['id']:"";?>" />
