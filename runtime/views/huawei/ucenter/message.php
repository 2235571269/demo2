...
<section class="uc_table">
	<table>
		...
		<tbody>
		    <?php $queryMessageList = Api::run('getUcenterMessageList');?>
			<?php foreach($items=$queryMessageList->find() as $key => $item){?>
			<tr>
				<td><h4><?php echo isset($item['title'])?$item['title']:"";?></h4></td>
				<td><?php echo isset($item['time'])?$item['time']:"";?></td>
				<td id="msg_id_<?php echo isset($item['id'])?$item['id']:"";?>"><?php if($queryMessageList->msg->is_read($item['id'])){?>已读<?php }else{?>未读<?php }?></td>
				<td class="blue">
					<a class='blue' href="<?php echo IUrl::creatUrl("/ucenter/message_del/id/".$item['id']."");?>">删除</a>
					<a class="blue" href="javascript:show_msg(<?php echo isset($item['id'])?$item['id']:"";?>);">查看</a>
				</td>
			</tr>
			<?php }?>
...
<?php echo $queryMessageList->getPageBar();?>
...
{
    $.getJSON("<?php echo IUrl::creatUrl("/ucenter/message_read");?>",{"id":id},function(json)
    {
