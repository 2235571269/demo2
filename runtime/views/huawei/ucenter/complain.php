...
<section class="uc_table">
	<table>
		<thead>
			<tr>
				<th>建议主题</th><th width="200">添加时间</th><th width="100">状态</th><th width="100">查看</th>
			</tr>
		</thead>
		<tbody>
		    <?php $querySuggestionList = Api::run('getUcenterSuggestion')?>
			<?php foreach($items=$querySuggestionList->find() as $key => $item){?>
			<script>var item<?php echo isset($key)?$key:"";?> = <?php echo JSON::encode($item);?>;</script>
			<tr>
				<td align="left"><?php echo isset($item['title'])?$item['title']:"";?></td>
				<td><?php echo isset($item['time'])?$item['time']:"";?></td>
				<td><?php if($item['re_content']!=''){?><span class="green">已回复</span><?php }else{?><span>未回复</span><?php }?></td>
				<td><a class="blue" href='javascript:show_msg(item<?php echo isset($key)?$key:"";?>)'>查看</a></td>
			</tr>
			<?php }?>
		</tbody>
	</table>
</section>
<?php echo $querySuggestionList->getPageBar();?>
