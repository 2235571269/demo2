...
<section class="user_form">
	<form action='<?php echo IUrl::creatUrl("/block/doPay");?>' method='post'>
		...
		<dl>
			...
			<dd>
				<?php if($paymentList){?>
				<?php foreach($items=$paymentList as $key => $item){?>
				<p>
					<label>
						<input class="radio" name="payment_id" title="<?php echo isset($item['name'])?$item['name']:"";?>" type="radio" value="<?php echo isset($item['id'])?$item['id']:"";?>">
