...
<section class="goods_list">
	<ul>
        <?php $favoriteObj = Api::run('getFavorite',IReq::get('cat_id'))?>
		<?php foreach($items=$favoriteObj->find() as $key => $item){?>
		<li>
			<a target="_blank" href="<?php echo IUrl::creatUrl("/site/products/id/".$item['goods_id']."");?>">
			...
			<div class="goods_list_btnbox">
				<div class="add_cart" onclick="joinCart_list(<?php echo isset($item['goods_id'])?$item['goods_id']:"";?>);">加入购物车</div>
				<div class="del_fav" onclick="delModel({link:'<?php echo IUrl::creatUrl("/ucenter/favorite_del/id/".$item['id']."");?>',msg:'是否取消收藏？'});">取消收藏</div>
			</div>
		</li>
		<?php }?>
	</ul>
</section>
<?php echo $favoriteObj->getPageBar();?>
