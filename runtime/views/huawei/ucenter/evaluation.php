...
<section class="uc_table">
	<table width="100%" cellpadding="0" cellspacing="0">
		...
		<tbody>
        	<?php $queryEvaluationList = Api::run('getUcenterEvaluation',0)?>
			<?php foreach($items=$queryEvaluationList->find() as $key => $item){?>
			<tr>
				<td><?php echo isset($item['order_no'])?$item['order_no']:"";?></td>
				<td><a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['goods_id']."");?>" target='_block'><?php echo isset($item['name'])?$item['name']:"";?><?php if($item['value']){?>【<?php echo isset($item['value'])?$item['value']:"";?>】<?php }?></a></td>
...
<?php echo $queryEvaluationList->getPageBar();?>
