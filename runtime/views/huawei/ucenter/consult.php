...
<section class="uc_table">
	<table>
		...
		<tbody>
		    <?php $queryConsultList = Api::run('getUcenterConsult')?>
			<?php foreach($items=$queryConsultList->find() as $key => $item){?>
			<script>var item<?php echo isset($key)?$key:"";?> = <?php echo JSON::encode($item);?>;</script>
			<tr>
				<td><a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['gid']."");?>" target="_black"><?php echo isset($item['name'])?$item['name']:"";?></a></td>
				<td><?php echo isset($item['question'])?$item['question']:"";?></td>
				<td><?php echo substr($item['time'],0,10);?></td>
				<td><?php if($item['status']==1){?><span class="blue" onclick='show_msg(item<?php echo isset($key)?$key:"";?>)'>查看</span><?php }else{?>未回复<?php }?></td>
			</tr>
			<?php }?>
		</tbody>
	</table>
</section>
<?php echo $queryConsultList->getPageBar();?>
