...
<body class="skin-blue fixed sidebar-mini" style="height: auto; min-height: 100%;">
	...
	<ul class="breadcrumb">
		...
		<li class="active">管理员导航<?php if(isset($this->navigationRow['id'])){?>编辑<?php }else{?>添加<?php }?></li>
	</ul>
</div>
<div class="content">
	<form action="<?php echo IUrl::creatUrl("/system/navigation_update");?>"  method="post" name="navigation_edit">
		...
		<table class="table form-table">
			...
			<tr>
				<th>导航名称：</th>
				<td>
					<input type='text' name='naviga_name' id='naviga_name' class='form-control' pattern='required' alt='请输入导航名称'/>
					<p class="help-block" id='unique'>* 管理员常用链接的名称</p>
				</td>
			</tr>
			<tr name="pwd">
				<th>url地址：</th>
				<td>
					<input type='text' class='form-control' name='url' id='url' pattern='required' alt='请输入url地址' />
					<p class="help-block">* 管理员常用链接的链接地址&nbsp;&nbsp;(完整路径)</p>
				</td>
			</tr>
...
formObj.init(<?php echo JSON::encode($this->navigationRow);?>);
</script>
        </div>
        <!--右侧内容 结束-->
