...
<div class="content">
	<form method='post' action='<?php echo IUrl::creatUrl("/market/cost_point_del");?>'>
		<table class="table list-table">
			...
			<caption>
                <a class="btn btn-default" onclick="event_link('<?php echo IUrl::creatUrl("/market/cost_point_edit");?>');">
                    <i class="fa fa-plus"></i>添加积分兑换
                </a>
			...
			<tbody>
				 <?php $queryObj=Api::run('getListByCostPoint');$resultData=$queryObj->find()?>
                 <?php foreach($items=$resultData as $key => $item){?>
				<tr>
                    <td><input type='checkbox' name='id[]' value='<?php echo isset($item["cid"])?$item["cid"]:"";?>' /></td>
                    <td><?php echo isset($item['cname'])?$item['cname']:"";?></td>
                    <td><a href='<?php echo IUrl::creatUrl("/site/products/id/".$item["id"]."");?>' target='_blank'><?php echo isset($item['name'])?$item['name']:"";?></a></td>
                    <td><?php echo isset($item['sort'])?$item['sort']:"";?></td>
                    <td><?php echo active::statusCostPoint($item);?></td>
                    <td>
                        <a href='<?php echo IUrl::creatUrl("/market/cost_point_edit/id/".$item["cid"]."");?>'>
                            <i class='operator fa fa-edit'></i>
                        </a>

                        <a href='javascript:void(0)' onclick="delModel({link:'<?php echo IUrl::creatUrl("/market/cost_point_del/id/".$item['cid']."");?>'});">
