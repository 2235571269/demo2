...
<div class="content">
	<form method='post' action='<?php echo IUrl::creatUrl("/market/ticket_excel");?>'>
		<table class="table list-table">
			...
			<caption>
                <a class="btn btn-default" href='<?php echo IUrl::creatUrl("/market/ticket_edit");?>'>
			...
			<tbody>
			    <?php $queryObj=Api::run('getListByTicket');$resultData=$queryObj->find()?>
				...
				<tr>
					...
					<td>
						<a href='<?php echo IUrl::creatUrl("/market/ticket_edit/id/".$item['id']."");?>' title="编辑">
							<i class='operator fa fa-edit'></i>
						</a>

						<a href='<?php echo IUrl::creatUrl("/market/ticket_more_list/ticket_id/".$item['id']."");?>' title="查看优惠券">
							<i class='operator fa fa-eye'></i>
						</a>

						<a href='javascript:create_dialog("<?php echo isset($item['id'])?$item['id']:"";?>");' title="生成优惠券">
							<i class='operator fa fa-money'></i>
						</a>

						<?php if($ticket_num > 0){?>
						<a href='javascript:void(0)' title="生成EXCEL" onclick="delModel({msg:'是否要生成excel表格？',link:'<?php echo IUrl::creatUrl("/market/ticket_excel/id/".$item['id']."");?>'});">
