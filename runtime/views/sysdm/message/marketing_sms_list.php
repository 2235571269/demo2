...
<div class="content">
	<table id="list_table" class="table list-table">
		...
		<tbody>
            <?php $queryObj=Api::run('getListByMarketing');$resultData=$queryObj->find()?>
            <?php foreach($items=$resultData as $key => $item){?>
			<tr>
				<td><input name="check[]" type="checkbox" value="<?php echo isset($item['id'])?$item['id']:"";?>" /></td>
				<td><?php echo isset($item['rev_info'])?$item['rev_info']:"";?></td>
				<td><?php echo isset($item['send_nums'])?$item['send_nums']:"";?></td>
				<td><?php echo isset($item['time'])?$item['time']:"";?></td>
				<td>
					<a href="<?php echo IUrl::creatUrl("/message/marketing_sms_show/id/".$item['id']."");?>"><i class='operator fa fa-eye'></i></a>
					<a onclick="delModel({link:'<?php echo IUrl::creatUrl("/message/marketing_sms_del/check/".$item['id']."");?>'})"><i class='operator fa fa-close'></i></a>
				</td>
			</tr>
			<?php }?>
		</tbody>
...
//发送营销短信后回调
function startMarketingSmsCallback(msg)
{
	if(msg == 'success')
	{
		tips('发送营销短信成功');
