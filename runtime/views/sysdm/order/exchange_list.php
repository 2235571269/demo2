...
<body class="skin-blue fixed sidebar-mini" style="height: auto; min-height: 100%;">
	...
	<ul class="breadcrumb">
		...
		<li class="active">换货申请列表</li>
	</ul>
</div>

<div class="content">
	<form name="orderForm" action="<?php echo IUrl::creatUrl("/order/exchange_doc_del");?>" method="post">
	<table class="table list-table">
		...
		<tbody>
			<?php $queryObj=Api::run('getExchangeList');$resultData=$queryObj->find()?>
			...
			<tr>
				...
				<td>
					<?php foreach($items=Api::run('getOrderGoodsRowById',array('id'=>$item["order_goods_id"])) as $key => $itemGoods){?>
					<?php $goods = JSON::decode($itemGoods['goods_array'])?>
					<p>
						<a href="<?php echo IUrl::creatUrl("/site/products/id/".$itemGoods['goods_id']."");?>" target="_blank"><?php echo isset($goods['name'])?$goods['name']:"";?> X <?php echo isset($itemGoods['goods_nums'])?$itemGoods['goods_nums']:"";?></a>
				...
				<td><?php echo Order_Class::refundmentText($item['status']);?></td>
				<td><a href="<?php echo IUrl::creatUrl("/order/exchange_doc_show/id/".$item['id']."");?>"><i class='operator fa fa-eye'></i></a></td>
			</tr>
			<?php }?>
		</tbody>
	</table>
