...
<div class="content">
	<form action="<?php echo IUrl::creatUrl("/member/group_save");?>" method="post">
		...
		<table class="table form-table">
			...
			<tr>
				<th>用户组：</th><td><input class="form-control" name="group_name" type="text" value="<?php echo isset($group['group_name'])?$group['group_name']:"";?>" pattern="required" placeholder="会员组名称" /></td>
			</tr>
			<tr>
				<th>折扣率：</th><td><input class="form-control w-auto" name="discount" type="text" value="<?php echo isset($group['discount'])?$group['discount']:"";?>" pattern="float" placeholder="请输入数字" />%<p class="help-block">折扣率，例如：如果输入90，表示该会员组可以以商品原价的90%购买（允许包含小数）。</p></td>
			</tr>
			<tr>
				<th>最小经验：</th><td><input class="form-control" name="minexp" onblur="check_exp();" type="text" value="<?php echo isset($group['minexp'])?$group['minexp']:"";?>" pattern="int" placeholder="填写整数值" /><p class="help-block">进入此会员组的经验值下限</p></td>
			</tr>
			<tr>
				<th>最大经验：</th>
				<td>
					<input class="form-control" name="maxexp" onblur="check_exp();" type="text" value="<?php echo isset($group['maxexp'])?$group['maxexp']:"";?>" pattern="int" alt="请填写一个整数值" />
